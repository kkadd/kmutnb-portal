...
export const ServiceManage = () => {
  ...
  const filteredServices = useMemo(() => {
    const searchService = service.filter((service) =>
      service.serviceName.toLowerCase().includes(filterValue.toLowerCase())
    );
    return searchService.filter(
      (service) => service.username == username || managementRole == "admin"
    );
  }, [filterValue, managementRole, service, username]);
      ...
      <div className="my-6 lg:px-6 max-w-[95rem] mx-auto w-full flex flex-col gap-4">
        ...
        <div className="flex justify-center items-center mt-10 w-full">
          <Pagination
            ...
            variant="flat"
            showControls
            total={Math.ceil(filteredServices.length / itemsPerPage)}
